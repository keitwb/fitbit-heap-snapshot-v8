...
import { Edge, HeapSnapshot, Node, NodeTypes, RawNode, RawEdge, ChromeDevSnapshot, nodeTypeList, edgeTypeList, SourcePosition, nodeTypeMap, edgeTypeMap, normalizedNodeTypes, GraphNodeAttributes, GraphEdgeAttributes } from './types';
...
function flattenEdges(graph: MultiDirectedGraph<GraphNodeAttributes, GraphEdgeAttributes>, stringMapper: StringMapper): number[] {
...
function flattenNodes(graph: MultiDirectedGraph<GraphNodeAttributes, GraphEdgeAttributes>, stringMapper: StringMapper): number[] {
...
export async function generateGraph(
  ...
  const dcg = new MultiDirectedGraph<GraphNodeAttributes, GraphEdgeAttributes>();
...
export function generateV8HeapSnapshot(graph: MultiDirectedGraph<GraphNodeAttributes, GraphEdgeAttributes>): ChromeDevSnapshot {
  const stringMapper = new StringMapper();

  const nodeList = flattenNodes(graph, stringMapper);
  const edgesList = flattenEdges(graph, stringMapper);

  sanityChecks(graph, nodeList, edgesList);
...
function sanityChecks(graph: MultiDirectedGraph<GraphNodeAttributes, GraphEdgeAttributes>, nodeList: number[], edgeList: number[]) {
